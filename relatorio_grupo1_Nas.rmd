...
Some values in the clinical_data are missing (NA). We need to remove or impute these values to avoid bias.

```{r}
sorted_missing = sort(percent_missing_clinical, decreasing = TRUE)

```
```{r}
# Selecionar apenas as colunas que possuem NAs
```
...
# Exibir os nomes das variáveis com valores ausentes
